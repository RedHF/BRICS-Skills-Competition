...
func MatchTrace(spec *content.TraceSpec, strokes [][][2]float64) bool {
	return len(FailedTraceStrokes(spec, strokes)) == 0
}

func FailedTraceStrokes(spec *content.TraceSpec, strokes [][][2]float64) []int {
	failed := []int{}
	total := 0
	for _, stroke := range strokes {
		total += len(stroke)
	}
	for i, guide := range spec.Strokes {
		if i >= len(strokes) || len(strokes) > len(spec.Strokes) || total > 8192 || !matchStroke(spec, guide, strokes[i]) {
...
func matchStroke(spec *content.TraceSpec, guide, drawn [][2]float64) bool {
	if len(drawn) < 2 {
		return false
	}
	guide = append([][2]float64{}, guide...)
	for j := range guide {
	...
	for j, p := range drawn {
		...
		if best > spec.Tolerance*1.6 || progress < previous-spec.Tolerance*2 || (j == 0 && progress > spec.Tolerance) {
			return false
		}
		if best > spec.Tolerance {
			outside++
		}
		previous = math.Max(previous, progress)
	}
	if outside*10 > len(drawn) || previous < lengths[len(lengths)-1]-spec.Tolerance {
		return false
	}
	return true
}
