...
func TestMemoryReferencesAndStoryFailAtLoadBoundary(t *testing.T) {
	for _, which := range []string{"missing_memory", "wrong_source", "missing_story", "bad_trace", "missing_art", "missing_failure", "missing_failure_reason", "empty_failure_line"} {
		t.Run(which, func(t *testing.T) {
			...
			case "missing_failure":
				c.FailureScene.Dialogue = nil
			case "missing_failure_reason":
				delete(c.FailureScene.FirstLines, "erosion_limit")
			case "empty_failure_line":
				c.FailureScene.Dialogue[1].Text = " "
			case "missing_art":
