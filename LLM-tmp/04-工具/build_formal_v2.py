...
OUT = r"C:\Users\21178\Desktop\files\BRICS-Skills-Competition\《檐下千秋》游戏策划案（正式文案·临时稿v3）.docx"
...
class Builder:
    ...
    def run(self, text, font="宋体", size=24, bold=False, color=None, footnote=None):
        ...
        if footnote is not None:
            r += ('<w:r><w:rPr><w:vertAlign w:val="superscript"/></w:rPr>'
    ...
    def body_para(self, text, first_line=True, align=None):
    ...
    def heading(self, text, level=1):
        """标题：黑体三号（32 半磅=16pt）加粗；level1 居中，level2 左对齐（黑体四号 28）。"""
        sz = 32 if level == 1 else 28
        ppr = '<w:pPr><w:spacing w:line="360" w:lineRule="auto" w:before="240" w:after="120"/>'
        if level == 1:
            ppr += '<w:jc w:val="center"/>'
        ppr += f'<w:outlineLvl w:val="{level-1}"/></w:pPr>'
        return self.para([self.run(text, font="黑体", size=sz, bold=True)], ppr)

    def render_text(self, text):
        """处理 {{关键词}} 标记：首次出现=蓝色+脚注；后续出现=仅蓝色。"""
        runs = []
        buf = ""
        i = 0
        while i < len(text):
            if text.startswith("{{", i):
                ...
                if kw in KEYWORDS:
                    if kw not in self.footnote_id_of:
                        ...
                        runs.append(self.run(kw, color="0000FF", footnote=fid))
                    else:
                        runs.append(self.run(kw, color="0000FF"))
                else:
                    runs.append(self.run(kw))
                i = end + 2
            else:
                buf += text[i]
                i += 1
...
b.spacer(3)
for txt in ("作品名称：《檐下千秋》", "队伍 ID：【待确认】", "组别：【待确认】", "日期：2026 年 8 月"):
    b.body.append(
...
b.body.append('<w:p><w:r><w:br w:type="page"/></w:r></w:p>')
...
# ===== 一、游戏概述与核心概念 =====
b.body.append(b.heading("一、游戏概述与核心概念", 1))
...
b.body.append(b.body_para("主题切入采用赛题维度二「建筑文化的空间叙事」。取材中国传统建筑承载「记忆」的文化内涵，包括风雨廊桥、社庙、戏楼、牌坊、斗拱、飞檐、藻井、{{通天塔}}等典型建筑与构件。"))
...
# ===== 二、核心玩法与机制设计 =====
b.body.append(b.heading("二、核心玩法与机制设计", 1))
...
# ===== 三、系统设计与世界观剧情 =====
b.body.append(b.heading("三、系统设计与世界观剧情", 1))
...
b.body.append(b.body_para("如果说要用一句话介绍《檐下千秋》的世界，那便是「檐下藏记忆，遗忘毁文明」。这是一个架空古风世界：那里的宫殿、廊桥、戏楼不只是砖瓦木石，而是会「记住」人的故事的建筑。玩家扮演的{{拓印}}师要做的，就是赶在记忆被彻底遗忘之前，把故事{{拓印}}下来、留存下去。"))
b.body.append(b.body_para("从时间上看，故事设定在一个架空古风王朝「墨痕王朝」。之所以采用「架空」而非绑定某一具体朝代，是为了在不失中国传统建筑神韵的前提下，为玩法与剧情留出创作空间：玩家在旅途中见到的风雨廊桥、社庙、戏楼、牌坊与{{通天塔}}，均以现实古建形制为蓝本，又经过艺术化加工，兼具真实感与想象力。"))
b.body.append(b.body_para("从地点上看，玩家的旅程自乡野一路通向皇城：起点是被{{白蚀}}侵蚀得只剩一半的风雨廊桥，接着是香火记忆所在的乡村社庙、锣鼓声里的城镇戏楼、铭刻功名的官道牌坊，最终抵达皇城深处的七层{{通天塔}}。每经过一座古建，玩家都会遇见一段等待被记住的故事，地理上的层层递进，也暗合文明由乡土走向庙堂的脉络。"))
b.body.append(b.body_para("这个世界最核心的设定是：建筑会「记住」事情。建造者的手艺、节庆的仪式、行人的悲欢，都会随年月沉淀进砖瓦木石之中，凝结成肉眼看不见的灵质——{{墨灵}}。这些被建筑保存的记忆统称「匠魂之忆」，而「建筑承载记忆、记忆维系文明」这一整套秩序，被后世称为{{建筑意}}。不妨这样理解：在这片大地上，一座有故事的老建筑是「活着」的，它记得谁曾来过、谁曾唱过、谁曾在檐下躲过雨。"))
b.body.append(b.body_para("然而，墨痕王朝正深陷一场被称为{{墨竭之劫}}的浩劫。当一段记忆被遗忘——也许是岁月冲刷，也许是人们不再提起——它就会从建筑中消散，并在消散之处凝结成灰白色的侵蚀物，也就是{{白蚀}}。更令人不安的是，{{白蚀}}会像瘟疫一样侵蚀邻近的建筑：建筑失忆、人们遗忘、记忆消散、{{白蚀}}蔓延，如此循环往复、愈演愈烈。被{{白蚀}}吞没的庙宇会褪色剥落，乡民也会渐渐忘记自己从何而来——这并非外敌入侵，而是「被遗忘」本身化成了可见的形状。"))
b.body.append(b.body_para("能对抗{{白蚀}}的，唯有{{拓印}}师的手艺——将建筑中的记忆原样「临摹」下来，封存为一张张{{墨灵符诏}}。但识海（承载记忆的心灵空间）容量有限：主角至多同时保留 5–7 道记忆。这意味着，每{{拓印}}一段新记忆，往往就要亲手抹去一段旧记忆——记住什么、遗忘什么没有标准答案，只有玩家自己的选择。这份两难，正是本作全部抉择玩法的起点。"))
b.body.append(b.body_para("你说的对，但是《檐下千秋》是由上海商学院自主研发的一款全新国风解密冒险游戏。游戏发生在一个被称作「墨痕王朝」的幻想世界，在这里，被古建选中的人将被授予「{{墨灵}}」，导引记忆之力。你将扮演一位名为「{{拓印}}师」的神秘角色，在自由的古建之旅中邂逅性格各异、记忆独特的古建们，和他们一起击败强敌{{白蚀}}，找回失散的文明记忆——同时，逐步发掘「{{墨竭之劫}}」的真相。"))
...
b.body.append(b.body_para("③ {{白蚀}}三阶段做成可交互视觉（浸 / 蚀 / 竭），服务 Task03 演示；"))
...
# ===== 五、可视化表达 =====
b.body.append(b.heading("五、可视化表达", 1))
...
# ===== 六、AI 赋能策划应用与协作记录 =====
b.body.append(b.heading("六、AI 赋能策划应用与协作记录", 1))
...
b.body.append(b.body_para("为充分体现人机协同、确保每一条创意均可追溯，本作品对全部 AI 使用过程实行完整留痕，具体规范如下："))
b.body.append(b.body_para("其一，完整保存对话记录。与 AI 的每一轮交互均以工具自带的导出功能留存原始对话，作为最核心的过程凭证；对无法导出的情形，则以全程截图作为补充依据，确保关键过程不缺失、可还原。"))
b.body.append(b.body_para("其二，保留修改版本对照。AI 生成的初稿与人工修订后的版本分别归档并互相对应，完整呈现每一版本的变化内容与修订理由，真实反映人机协作的演进过程。"))
b.body.append(b.body_para("其三，标注创意来源。策划案所采纳的每一条核心创意均注明其产生方式——由 AI 生成、由人工构思，或经双方协商共同形成，以明确区分 AI 产出与人工决策的边界。"))
b.body.append(b.body_para("其四，登记逐轮协作记录。每次协作在记录表中如实登记日期、所用工具、本轮目标、提示词原文、AI 输出摘要、人工修订要点及成果回流情况，形成时序完整、便于核查的协作档案。"))
b.body.append(b.body_para("其五，落实存档纪律。全部记录随做随存、杜绝事后补录，以保证时间信息真实可信；留存材料在归档前隐去与竞赛无关的个人信息，涉及账号密码等敏感内容一律进行脱敏处理。"))
b.body.append(b.body_para("以上留痕材料最终统一汇总为《AI 协作记录》，随策划案一并提交，系统呈现本作品在 AI 辅助下由概念形成方案的全过程。"))

# ===== 七、文档规范与提交自查清单 =====
b.body.append(b.heading("七、文档规范与提交自查清单", 1))
...
# ---------------------------------------------------------------------------
# 组装 document.xml（sectPr 引用骨架中的 footer1.xml = rId9）
# ---------------------------------------------------------------------------
document_xml = (
...
# ---------------------------------------------------------------------------
# 打包：以源 docx 为骨架，仅替换 document.xml，新增 footnotes.xml 并注册
# ---------------------------------------------------------------------------
with zipfile.ZipFile(SRC) as z:
    parts = {n: z.read(n) for n in z.namelist()}

# 1) 注册 footnotes.xml 到 [Content_Types].xml
...
# 2) 注册 footnotes.xml 关系到 document.xml.rels
...
# 3) 替换 document.xml 与新增 footnotes.xml
